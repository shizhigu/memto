/**
 * Tiny TUI helpers for the bundled examples.
 * Aesthetic borrows from `hermes status` — diamond section markers, runtime-
 * tinted tags, dim meta — but with a green palette so the two tools are
 * visually distinct.
 */
...
export const c = {
  ...
  gold: (s: string) => `${rgb(245, 185, 76)}${s}${reset}`, // kept for runtime accents
};
...
export function runtimeTag(runtime: string): string {
  ...
  const label = ` ${runtime.padEnd(11)} `;
  return `${bgRgb(30, 24, 18)}${rgb(r, g, b)}${label}${reset}`;
}
...
/** `┌─ title ────────┐` top line, content lines with side rails, bottom line. */
export function box(title: string, lines: string[], width = 78): string {
  const top = `┌─ ${c.green(title)} ${'─'.repeat(Math.max(0, width - visibleLength(title) - 5))}┐`;
  const rail = c.slate('│');
  const body = lines.map((l) => {
    const pad = ' '.repeat(Math.max(0, width - visibleLength(l) - 2));
    return `${rail} ${l}${pad}${rail}`;
  });
  const bot = c.slate(`└${'─'.repeat(width)}┘`);
  return [c.slate(top)].concat(body).concat([bot]).join('\n');
}
...
/** Render the big "memto" banner using cfonts. Matches assets/banner.svg. */
export function banner(subtitle?: string): string {
  if (!useColor) {
    return `\n  memto${subtitle ? '  ' + subtitle : ''}\n`;
  }
  // `block` font fits 80-col terminals; `huge` (used in the SVG banner) wraps.
